...
public partial class AsteroidsOutOfBoundsSystem : SystemBase
{
    ...
    protected override void OnUpdate()
    {
        ...
        .ForEach((Entity entity, int entityInQueryIndex, in Translation position) =>
        {
            ...
            if (Mathf.Abs(position.Value.x) > settings.levelWidth/2 + 100 ||
                Mathf.Abs(position.Value.y) > settings.levelHeight/2 + 1 ||
                Mathf.Abs(position.Value.z) > settings.levelDepth/2 + 100)
            {
                //If it is out of bounds we add the DestroyTag component to the entity and return
